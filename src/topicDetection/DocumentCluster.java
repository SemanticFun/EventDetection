...
public class DocumentCluster {
	...
	public HashMap<String, Document> FPdocs = new HashMap<String, Document>();
	public HashMap<String, Document> FNdocs = new HashMap<String, Document>();
	...
	public boolean deserialize(BufferedReader in, boolean readDocContent) throws Exception {
		...
		new WriteConsole(Arrays.asList(tokens) + "\n");
		for (int i = 0; i < tokens.length; i++) {
			tokens[i] = tokens[i].trim();
			if (tokens[i].length() > 0) {
